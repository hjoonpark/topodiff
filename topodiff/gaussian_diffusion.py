...
class GaussianDiffusion:
    ...
    def condition_mean(self, cond_fn_1, cond_fn_2, p_mean_var, x, cons, loads, BCs, t, model_kwargs=None):
        """
        ...
        gradient_2 = cond_fn_2(x, self._scale_timesteps(t), **model_kwargs)
        # Maximum Level of Noise for floating material classifier: 700
        new_mean = (
            p_mean_var["mean"].float() + p_mean_var["variance"] * gradient_2.float() * (0 if t_rs < 300 else 1)
        )

        full_arr_reg = th.cat((new_mean, cons, loads, BCs), dim = 1)
        gradient_1 = cond_fn_1(full_arr_reg, self._scale_timesteps(t), **model_kwargs)
        # Maximum Level of Noise for floating material classifier: 1000
        new_mean = (
            new_mean + p_mean_var["variance"] * gradient_1.float()
        )
    ...
    def p_sample(
        ...
        if cond_fn_1 is not None and cond_fn_2:
            out["mean"] = self.condition_mean(
                cond_fn_1, cond_fn_2, out, x, cons, loads, BCs, t, model_kwargs=model_kwargs
            )
            temp = out["mean"]
